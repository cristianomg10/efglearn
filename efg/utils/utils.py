...
def min_u(granules, indices, m):
    ...
    for i in indices:
        for j in range(0, m):
            if granules[i].output_granules[j].u < min:
                min = granules[i].output_granules[j].u
                index_min = i

    return min

def min_u_mp(granules, indices, m):
    """
    Used for convex hull
    :param granules:
    :param indices:
    ...
    if len(indices) == 1: return granules[indices[0]].oGranules[0].u

    for i in indices:
        for j in range(0, m):
            if granules[i].output_granules[j].midpoint() < min:
                min = granules[i].output_granules[j].midpoint()
                index_min = i

    return min

def mean_min_u(granules, indices, m):
    """
    Used for convex hull
    :param granules:
    :param indices:
    ...
    for i in indices:
        for j in range(0, m):
            min.append(granules[i].output_granules[j].u)

    return np.mean(min)

def wmean_min_u(granules, indices, m, S):
    """
    Used for convex hull
    :param granules:
    :param indices:
    ...
    for i in indices:
        for j in range(0, m):
            min.append(granules[i].output_granules[j].u * S[i])
...
def max_U(granules, indices, m):
    ...
    for i in indices:
        for j in range(0, m):
            if granules[i].output_granules[j].U > max:
                max = granules[i].output_granules[j].U
                index_max = i

    return max

def max_U_mp(granules, indices, m):
    """
    Used for convex hull
    :param granules:
    :param indices:
    ...
    if len(indices) == 1: return granules[indices[0]].output_granules[0].U

    for i in indices:
        for j in range(0, m):
            if granules[i].output_granules[j].midpoint() > max:
                max = granules[i].output_granules[j].midpoint()
                index_max = i

    return max

def mean_max_U(granules, indices, m):
    """
    Used for convex hull
    :param granules:
    :param indices:
    ...
    for i in indices:
        for j in range(0, m):
            max.append(granules[i].output_granules[j].U)

    return np.mean(max)

def wmean_max_U(granules, indices, m, S):
    """
    Used for convex hull
    :param granules:
    :param indices:
    ...
    for i in indices:
        for j in range(0, m):
            max.append(granules[i].output_granules[j].U * S[i])
...
def sum_dot_md(s, i, q):
    """
    Sum the items of a list multiplied by a factor from q.
    Used for calculation of affine functions
    :param s:
    :param i:
    :param q:
